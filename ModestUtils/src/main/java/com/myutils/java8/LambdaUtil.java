...
public class LambdaUtil {
	...
	public static List<Employee> filter(List<Employee> list) {
		// List<com.myutils.utils.Employee> newList = list.stream().filter(item ->
		// item.getSalary().compareTo(new BigDecimal(10000)) > 0 &&
		// !item.getUnitName().equals("项目经理")).collect(Collectors.toList());
	...
	public static List<String> toListStr(List<Employee> list) {
		// 写法一
		// List<String> newList = list.stream().map(com.myutils.utils.Employee ::
		// getEmpName).collect(Collectors.toList());
		// 写法二
